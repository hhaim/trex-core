...
trex_i40e_fdir_setup(struct i40e_pf *pf)
{
	...
	pf->fdir.prg_pkt_fdir = mz->addr;
	pf->fdir.dma_addr_fdir = mz->iova;
 ...
 */
void
trex_i40e_fdir_teardown(struct i40e_pf *pf)
{
	struct i40e_hw *hw = I40E_PF_TO_HW(pf);
	struct i40e_vsi *vsi;
...
static inline int
i40e_check_fdir_flex_payload(const struct trex_rte_eth_flex_payload_cfg *flex_cfg)
{
...
i40e_check_fdir_flex_conf(const struct i40e_adapter *adapter,
			  const struct trex_rte_eth_fdir_flex_conf *conf)
{
	const struct trex_rte_eth_flex_payload_cfg *flex_cfg;
	const struct trex_rte_eth_fdir_flex_mask *flex_mask;
	uint16_t mask_tmp;
	uint8_t nb_bitmask;
	uint16_t i, j;
	int ret = 0;
...
i40e_set_flx_pld_cfg(struct i40e_pf *pf,
			 const struct trex_rte_eth_flex_payload_cfg *cfg)
{
	struct i40e_hw *hw = I40E_PF_TO_HW(pf);
...
i40e_set_flex_mask_on_pctype(struct i40e_pf *pf,
		enum i40e_filter_pctype pctype,
		const struct trex_rte_eth_fdir_flex_mask *mask_cfg)
{
	struct i40e_hw *hw = I40E_PF_TO_HW(pf);
 ...
 */
static void
trex_i40e_fdir_rx_proc_enable(struct rte_eth_dev *dev, bool on)
{
	int32_t i;
...
static void copy_fdir_flex_cfg(struct trex_rte_eth_fdir_flex_conf *to,
							   struct rte_eth_fdir_flex_conf *from){
	if (sizeof(struct rte_eth_fdir_flex_conf) != sizeof(struct trex_rte_eth_fdir_flex_conf)){
	   assert(0);
	}
	memcpy((void *)to,(void *)from,sizeof(struct rte_eth_fdir_flex_conf));
}
 ...
 */
int
trex_i40e_fdir_configure(struct rte_eth_dev *dev)
{
	...
	*/
	...
	struct trex_rte_eth_fdir_flex_conf trex_conf;

	copy_fdir_flex_cfg(&trex_conf, conf);

	ret = i40e_check_fdir_flex_conf(pf->adapter, &trex_conf);
	if (ret < 0) {
	...
	if (!pf->support_multi_driver) {
		...
		for (i = 0; i < conf->nb_payloads; i++)
			i40e_set_flx_pld_cfg(pf, &trex_conf.flex_set[i]);
		/* configure flex mask*/
		for (i = 0; i < conf->nb_flexmasks; i++) {
			if (hw->mac.type == I40E_MAC_X722) {
				...
				pctype = (enum i40e_filter_pctype)
					  i40e_read_rx_ctl(hw,
						...
						pf->adapter,
						trex_conf.flex_mask[i].flow_type)));
			} else {
	...
	trex_i40e_fdir_rx_proc_enable(dev, 1);

	return ret;
}

static inline int
i40e_fdir_fill_eth_ip_head(const struct trex_rte_eth_fdir_input *fdir_input,
...
i40e_fdir_construct_pkt(struct i40e_pf *pf,
			     const struct trex_rte_eth_fdir_input *fdir_input,
			     unsigned char *raw_pkt)
{
...
static int
trex_i40e_sw_fdir_filter_del(struct i40e_pf *pf, struct i40e_fdir_input *input)
{
	struct i40e_fdir_info *fdir_info = &pf->fdir;
	struct i40e_fdir_filter *filter;
	int ret;
 ...
 */
int
trex_i40e_add_del_fdir_filter(struct rte_eth_dev *dev,
			 const struct trex_rte_eth_fdir_filter *filter,
			 bool add)
{
	struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
 ...
 */
static int
trex_i40e_flow_add_del_fdir_filter(struct rte_eth_dev *dev,
			      const struct i40e_fdir_filter_conf *filter,
			      bool add)
{
	struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
*/
	...
	if (add) {
		...
		ret = trex_i40e_sw_fdir_filter_del(pf, &node->fdir.input);
	}
...
i40e_fdir_info_get_flex_set(struct i40e_pf *pf,
			struct trex_rte_eth_flex_payload_cfg *flex_set,
			uint16_t *num)
{
	struct i40e_fdir_flex_pit *flex_pit;
	struct trex_rte_eth_flex_payload_cfg *ptr = flex_set;
...
i40e_fdir_info_get_flex_mask(struct i40e_pf *pf,
			struct trex_rte_eth_fdir_flex_mask *flex_mask,
			uint16_t *num)
{
	struct i40e_fdir_flex_mask *mask;
	struct trex_rte_eth_fdir_flex_mask *ptr = flex_mask;
	uint16_t flow_type;
	uint8_t i, j;
 ...
 */
static void
trex_i40e_fdir_info_get(struct rte_eth_dev *dev, struct trex_rte_eth_fdir_info *fdir)
{
	struct i40e_pf *pf = I40E_DEV_PRIVATE_TO_PF(dev->data->dev_private);
	struct i40e_hw *hw = I40E_PF_TO_HW(pf);
 ...
 */
static void
trex_i40e_fdir_stats_get(struct rte_eth_dev *dev, struct trex_rte_eth_fdir_stats *stat)
{
	struct i40e_pf *pf = I40E_DEV_PRIVATE_TO_PF(dev->data->dev_private);
	struct i40e_hw *hw = I40E_PF_TO_HW(pf);
	uint32_t fdstat;
 ...
 */
static int
trex_i40e_parse_input_set(uint64_t *inset,
		     enum i40e_filter_pctype pctype,
		     enum trex_rte_eth_input_set_field *field,
		     uint16_t size)
{
	uint16_t i, j;
	int ret = -EINVAL;
	...
	static const struct {
		...
		{TREX_RTE_ETH_INPUT_SET_NONE, I40E_INSET_NONE},
		{TREX_RTE_ETH_INPUT_SET_L2_SRC_MAC, I40E_INSET_SMAC},
		{TREX_RTE_ETH_INPUT_SET_L2_DST_MAC, I40E_INSET_DMAC},
		{TREX_RTE_ETH_INPUT_SET_L2_OUTER_VLAN, I40E_INSET_VLAN_OUTER},
		{TREX_RTE_ETH_INPUT_SET_L2_INNER_VLAN, I40E_INSET_VLAN_INNER},
		{TREX_RTE_ETH_INPUT_SET_L2_ETHERTYPE, I40E_INSET_LAST_ETHER_TYPE},
		{TREX_RTE_ETH_INPUT_SET_L3_SRC_IP4, I40E_INSET_IPV4_SRC},
		{TREX_RTE_ETH_INPUT_SET_L3_DST_IP4, I40E_INSET_IPV4_DST},
		{TREX_RTE_ETH_INPUT_SET_L3_IP4_TOS, I40E_INSET_IPV4_TOS},
		{TREX_RTE_ETH_INPUT_SET_L3_IP4_PROTO, I40E_INSET_IPV4_PROTO},
		{TREX_RTE_ETH_INPUT_SET_L3_IP4_TTL, I40E_INSET_IPV4_TTL},
		{TREX_RTE_ETH_INPUT_SET_L3_SRC_IP6, I40E_INSET_IPV6_SRC},
		{TREX_RTE_ETH_INPUT_SET_L3_DST_IP6, I40E_INSET_IPV6_DST},
		{TREX_RTE_ETH_INPUT_SET_L3_IP6_TC, I40E_INSET_IPV6_TC},
		...
		{TREX_RTE_ETH_INPUT_SET_L4_UDP_SRC_PORT, I40E_INSET_SRC_PORT},
		{TREX_RTE_ETH_INPUT_SET_L4_TCP_SRC_PORT, I40E_INSET_SRC_PORT},
		{TREX_RTE_ETH_INPUT_SET_L4_SCTP_SRC_PORT, I40E_INSET_SRC_PORT},
		{TREX_RTE_ETH_INPUT_SET_L4_UDP_DST_PORT, I40E_INSET_DST_PORT},
		{TREX_RTE_ETH_INPUT_SET_L4_TCP_DST_PORT, I40E_INSET_DST_PORT},
		{TREX_RTE_ETH_INPUT_SET_L4_SCTP_DST_PORT, I40E_INSET_DST_PORT},
		{TREX_RTE_ETH_INPUT_SET_L4_SCTP_VERIFICATION_TAG,
			I40E_INSET_SCTP_VT},
		{TREX_RTE_ETH_INPUT_SET_TUNNEL_L2_INNER_DST_MAC,
		...
		{TREX_RTE_ETH_INPUT_SET_TUNNEL_GRE_KEY, I40E_INSET_TUNNEL_ID},
		{TREX_RTE_ETH_INPUT_SET_FLEX_PAYLOAD_1ST_WORD,
		...
		{TREX_RTE_ETH_INPUT_SET_FLEX_PAYLOAD_8TH_WORD,
...
static int
trex_i40e_fdir_filter_inset_select(struct i40e_pf *pf,
			 struct trex_rte_eth_input_set_conf *conf)
{
	struct i40e_hw *hw = I40E_PF_TO_HW(pf);
	enum i40e_filter_pctype pctype;
	...
	ret = trex_i40e_parse_input_set(&input_set, pctype, conf->field,
				   conf->inset_size);
	if (ret) {
...
i40e_fdir_filter_set(struct rte_eth_dev *dev,
		     struct trex_rte_eth_fdir_filter_info *info)
{
	struct i40e_pf *pf = I40E_DEV_PRIVATE_TO_PF(dev->data->dev_private);
	int ret = 0;
	...
	case TREX_RTE_ETH_FDIR_FILTER_INPUT_SET_SELECT:
		ret = trex_i40e_fdir_filter_inset_select(pf,
 ...
 */
int
trex_i40e_fdir_ctrl_func(struct rte_eth_dev *dev,
		       enum trex_rte_filter_op filter_op,
		       void *arg)
{
	struct i40e_pf *pf = I40E_DEV_PRIVATE_TO_PF(dev->data->dev_private);
	int ret = 0;
	...
	if (filter_op == TREX_RTE_ETH_FILTER_NOP)
		return 0;

	if (arg == NULL && filter_op != TREX_RTE_ETH_FILTER_FLUSH)
		return -EINVAL;

	switch (filter_op) {
	case TREX_RTE_ETH_FILTER_ADD:
		ret = trex_i40e_add_del_fdir_filter(dev,
			(struct trex_rte_eth_fdir_filter *)arg,
			TRUE);
		break;
	case TREX_RTE_ETH_FILTER_DELETE:
		ret = trex_i40e_add_del_fdir_filter(dev,
			(struct trex_rte_eth_fdir_filter *)arg,
			FALSE);
		break;
	case TREX_RTE_ETH_FILTER_FLUSH:
		ret = trex_i40e_fdir_flush(dev);
		break;
	case TREX_RTE_ETH_FILTER_INFO:
		trex_i40e_fdir_info_get(dev, (struct trex_rte_eth_fdir_info *)arg);
		break;
	case TREX_RTE_ETH_FILTER_SET:
		ret = i40e_fdir_filter_set(dev,
			(struct trex_rte_eth_fdir_filter_info *)arg);
		break;
	case TREX_RTE_ETH_FILTER_STATS:
		trex_i40e_fdir_stats_get(dev, (struct trex_rte_eth_fdir_stats *)arg);
		break;
	default:
...
void
trex_i40e_fdir_filter_restore(struct i40e_pf *pf)
{
	...
	TAILQ_FOREACH(f, fdir_list, rules)
		trex_i40e_flow_add_del_fdir_filter(dev, &f->fdir, TRUE);
 ...
 */
static int
trex_i40e_ethertype_filter_handle(struct rte_eth_dev *dev,
				enum trex_rte_filter_op filter_op,
				void *arg)
{
	struct i40e_pf *pf = I40E_DEV_PRIVATE_TO_PF(dev->data->dev_private);
		     ...
		     void *arg)
{
    ...
    switch (filter_type) {
	case TREX_RTE_ETH_FILTER_ETHERTYPE:
		ret = trex_i40e_ethertype_filter_handle(dev, filter_op, arg);
		break;
	case TREX_RTE_ETH_FILTER_FDIR:
		ret = trex_i40e_fdir_ctrl_func(dev, filter_op, arg);
		break;
	default:
