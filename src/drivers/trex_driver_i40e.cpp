...
int CTRexExtendedDriverBase40G::dump_fdir_global_stats(CPhyEthIF * _if, FILE *fd)
{
    ...
    ret = trex_i40e_rte_eth_dev_filter_ctrl(repid, RTE_ETH_FILTER_FDIR, RTE_ETH_FILTER_STATS, (void*)&stat);
    if (ret == 0) {
        if (fd)
...
int CTRexExtendedDriverBase40G::verify_fw_ver(tvpid_t   tvpid) {
    ...
    if ((version > 0x6000) && get_dpdk_mode()->is_drop_rx_queue_needed()) {
      ...
      if (!rfile.fail()) {
        ...
        printf("Port (%d) firmware version 0x%x is not eligible for DDP upgrade \n", int(repid), version);
    }
