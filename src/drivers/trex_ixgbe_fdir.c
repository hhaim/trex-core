...
static void trex_ixgbe_fdir_info_get(struct rte_eth_dev *dev,
			struct rte_eth_fdir_info *fdir_info);
static void trex_ixgbe_fdir_stats_get(struct rte_eth_dev *dev,
...
static int
trex_ixgbe_fdir_set_input_mask(struct rte_eth_dev *dev)
{
	enum rte_fdir_mode mode = dev->data->dev_conf.fdir_conf.mode;
...
int
trex_ixgbe_fdir_set_flexbytes_offset(struct rte_eth_dev *dev,
				uint16_t offset)
{
	struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	uint32_t fdirctrl;
		    ...
		    const struct rte_eth_fdir_masks *input_mask)
{
	...
	return trex_ixgbe_fdir_set_input_mask(dev);
}
...
static int
trex_ixgbe_fdir_filter_program(struct rte_eth_dev *dev,
			  struct ixgbe_fdir_rule *rule,
			  bool del,
			  bool update)
{
			  ...
			  bool update)
{
	...
	return trex_ixgbe_fdir_filter_program(dev, &rule, del, update);
}
...
static void
trex_ixgbe_fdir_info_get(struct rte_eth_dev *dev, struct rte_eth_fdir_info *fdir_info)
{
	struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	struct ixgbe_hw_fdir_info *info =
			IXGBE_DEV_PRIVATE_TO_FDIR_INFO(dev->data->dev_private);
...
static void
trex_ixgbe_fdir_stats_get(struct rte_eth_dev *dev, struct rte_eth_fdir_stats *fdir_stats)
{
	struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	struct ixgbe_hw_fdir_info *info =
		IXGBE_DEV_PRIVATE_TO_FDIR_INFO(dev->data->dev_private);
			...
			enum rte_filter_op filter_op, void *arg)
{
	...
	case TREX_RTE_ETH_FILTER_ADD:
		...
		break;
	case TREX_RTE_ETH_FILTER_UPDATE:
		ret = ixgbe_add_del_fdir_filter(dev,
...
static int
trex_ixgbe_add_del_ethertype_filter(struct rte_eth_dev *dev,
...
void
trex_ixgbe_fdir_filter_restore(struct rte_eth_dev *dev)
{
	struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
int
trex_ixgbe_clear_all_fdir_filter(struct rte_eth_dev *dev)
{
	struct ixgbe_hw_fdir_info *fdir_info =
		IXGBE_DEV_PRIVATE_TO_FDIR_INFO(dev->data->dev_private);
	struct ixgbe_fdir_filter *fdir_filter;
...
static int
trex_ixgbe_get_ethertype_filter(struct rte_eth_dev *dev,
			struct trex_rte_eth_ethertype_filter *filter)
{
	struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	struct ixgbe_filter_info *filter_info =
 ...
 */
static int
trex_ixgbe_ethertype_filter_handle(struct rte_eth_dev *dev,
				...
				void *arg)
{
	...
	switch (filter_op) {
	case TREX_RTE_ETH_FILTER_ADD:
		ret = trex_ixgbe_add_del_ethertype_filter(dev,
			(struct trex_rte_eth_ethertype_filter *)arg,
			TRUE);
		break;
	case TREX_RTE_ETH_FILTER_DELETE:
		ret = trex_ixgbe_add_del_ethertype_filter(dev,
			...
			FALSE);
		break;
	case TREX_RTE_ETH_FILTER_GET:
		ret = trex_ixgbe_get_ethertype_filter(dev,
			(struct trex_rte_eth_ethertype_filter *)arg);
		break;
	default:
...
int trex_ixgbe_rte_eth_dev_filter_ctrl(uint16_t port_id, 
	...
	case TREX_RTE_ETH_FILTER_ETHERTYPE:
		ret = trex_ixgbe_ethertype_filter_handle(dev, filter_op, arg);
		break;
	case TREX_RTE_ETH_FILTER_FDIR:
		ret = trex_ixgbe_fdir_ctrl_func(dev, filter_op, arg);
		break;
	default:
