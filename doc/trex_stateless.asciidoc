...
:github_bird_path: https://github.com/cisco-system-traffic-generator/trex-core/tree/master/scripts/bird
...
:toclevels: 6
:doc_console: https://trex-tgn.cisco.com/trex/doc/trex_console.html
...
This feature is a combination of software and hardware NIC’s IEEE 1588 support. 
...
see here for link:trex_console.html[console tutorial]
...
==== Overview
link:https://bird.network.cz/[Bird Internet Routing Daemon] is a project aimed to develop a fully functional Linux dynamic IP routing daemon. It was integrated into TRex to run alongside in order to utilize its features together with Python automation API.
...
==== FAQ
You can find the Bird FAQ here: link:trex_faq.html#_bird_integration[Bird's FAQ]

==== High level Features 

* BIRD 2.0 supports the following protocols: link:https://bird.network.cz/?get_doc&f=bird.html&v=20[Bird Protocols]
...
The Bird process (`PyBird Server`) runs on it's own namespace on a TRex machine. It uses veths to communicate with the default namespace and transfer routes/data to TRex and from there to the DUT.
A single Bird process runs, but in this process we can simulate multiple instances of routing protocols. In the following sections, we will see how to run multiple BGP instances.
...
image::images/TRex_Bird_General_Scheme_With_ips.png[title="TRex Bird General Scheme",align="left",width={p_width}, link="images/TRex_Bird_General_Scheme_With_ips.png"]


As aforementioned, Bird uses the TRex Linux namespace feature, hence the first step is to enable it.
If you want to read more about the Linux stack in TRex, please refer to: link:trex_manual.html#stacks[Linux Stack].
The  *trex_cfg.yaml* file should contain "stack: linux_based":
...
- Next, we run TRex in in software mode (with one or multiple cores) and set the Bird server flag:
...
In another shell, let's run the following script: link:{github_stl_examples_path}/add_bird_node.py[stl/add_bird_node.py]

Let's take a look at a simplified version of the script. For more details check: link:trex_stateless.html#_tutorial_simple_bird_node_adding[Simple Bird node adding]
...
c.disconnect()
----
<1> Setting TRex on service mode.
<2> Use BirdCFGCreator in order to create a new Bird configuration
<3> Setting the configuration we just created.
<4> Create a Bird node
<5> dot1Q is a configuration of veth not namespace.
<6> Ensure Bird works on the protocols we just set.
<7> Exit the PyBirdClient gracefully.
...
[NOTE]
=====================================================================
The BGP neighboring between 1.1.1.3 and 1.1.1.1 in the router is in active
because the router is not configured with dot1Q encapsulation, while the 
PyBird instance is.
=====================================================================
...
All the tutorials are based on the topology xref:Bird_topo[here] +
...
4) Let's add a Bird node with the required parameters: (need at least aport and MAC address):
[source,bash]
----
...
Bird Nodes Information
    ...
    Node MAC      |   ipv4 address    |    ipv4 subnet    |   ipv6 enabled    |   ipv6 address    |
------------------+-------------------+-------------------+-------------------+-------------------+
00:00:00:00:00:06 |      1.1.1.3      |        24         |       False       |         -         |


    ipv6 subnet    |       vlans       |       tpids       
-------------------+-------------------+------------------
         -         |         -         |         -         
----
...
my_bgp2    BGP        ---        start  10:07:55.295  Active        Socket: Connection closed
static1    Static     master4    up     10:07:55.295
----

[NOTE]
=====================================================================
The second BGP instance is *Active* but not *Established*. This is 
because we didn't add a Bird node in that namespace.
=====================================================================
...
==== Tutorial: Bird with IPv6

We can run BGP on top of IPv6 as well as on top of IPv4. We are going to show here a simple Bird BGP configuration for IPv6. Of course, the router
needs to be configured with IPv6 as well.
...
==== Tutorial: Leveraging the Bird configuration files

Bird configuration files offer a full range of possibilities. They are very close to a programming language and we can define 
in them functions, filters, variables etc.

*Goal*::
    Export only locally originating routes in our BGP instances.

In the previous examples we explored the possibility of running two BGP instances. We saw however that both the instances export all the routes.
For example let's say we have the following IPv4 routes file:
...
What if we want to publish only the locally originating routes? Meaning the 1.1.1.3 node will publish only 42.42.42.1/32 since it originates at 1.1.1.3
while, the 1.1.2.3 node will only publish 42.42.42.2/32?

We can achieve the required quite easily, if we slightly modify our BGP conf file to the following:

link:{github_bird_path}/conf/bgp_filter.conf[bird/conf/bgp_filter.conf]
...
TRex supports all the configuration files that Bird offers. To leverage these configuration files at best, please refer to the Bird documentation.
Complex traffic engineering can be achieved quite easily using the correct attributes, functions and filters that the Bird configuration files can offer.
...
c.connect()
c.acquire(ports = my_ports)
c.set_port_attr(promiscuous=True)                      <2>
c.set_service_mode(ports = my_ports, enabled = True)   <3>

c.set_bird_node(node_port      = 0,                    <4>
                mac            = "00:00:00:01:00:07",  <5>
                ipv4           = "1.1.1.3",            <6>
                ipv6_enabled   = True,                 
                ipv4_subnet    = 24)


c.wait_for_protocols(['my_bgp1'])                      <7>
c.disconnect()
----
<1> We can use STL or ASTF clients.
<2> Bird requires promiscuous mode applied. It also requires multicast, depending on protocol.
<3> Service mode must be set.
<4> The physical port to which the node will be attached.
<5> The Mac address for the new Bird node, needs to be unique.
<6> The IPv4 address for the new node, MUST be on the same subnet with DUT interface.
<7> Busy wait until Bird confirms connection established on "my_bgp1" protocol.

WARNING: In case we add another Bird node with different parameters but the same Mac address an exception will be raised. Mac addresses must be unique.
...
*Goal*:: Show how to create the final `bird.conf` file with the routing protocols and the routes themselves (as static routes).
...
The BirdCFGCreator SDK is located here: link:https://trex-tgn.cisco.com/trex/doc/cp_stl_docs/api/bird_cfg_creator.html[Stateless Python API - Bird CFG Creator]
...
[NOTE]
=====================================================================
The configuration file must be valid according to link:https://bird.network.cz/?get_doc&v=20&f=bird-3.html[Bird's Documentation]
=====================================================================
...
List of supported protocols: link:https://bird.network.cz/?get_doc&v=20&f=bird-6.html[Bird's Protocols Documentation]
...
Simple routes are static routes with a gateway/next_hop parameter, that can be an IP or an interface name. These are the routes we saw until now.
Extended routes are generic routes, where the next_hop value can contain all types of attributes. Let us look at a couple of examples.
...
Once we have added our protocols and our routes, we can build the final configuration file, prior to sending it to the PyBird Server using
the PyBirdClient.
...
*Goal*:: Show the API for communicating with PyBird Server located on TRex machine using JSON-RPC2 over ZMQ.

For the SDK of PyBirdClient refer to: link:https://trex-tgn.cisco.com/trex/doc/cp_stl_docs/api/pybird_client.html[Stateless Python API - PyBird Client]
...
[source,python]
----
pybird_c = PyBirdClient(ip='localhost', port=4509)  <1>
pybird_c.connect()                                  <2>
pybird_c.acquire()                                  <3>

pybird_c.get_config()                               <4>
pybird_c.get_protocols_info()

pybird_c.set_empty_config()                         <5>

pybird_c.release()                                  <6>
pybird_c.disconnect()                               <7>
----
<1> Create a PyBird client, which should connect to the PyBird Server on local host at port 4509.
<2> Connect to the PyBird server, required for every method.
<3> Acquire handler from server, required for command methods.
<4> Get configuration of the server, a query method.
<5> Setting an empty config file, a command method.
<6> Release the acquired handler.
...
In case of using a query like: `get_config`, connecting will be enough. In case of using a command like: `set_config` you have to connect and acquire.
The TRex client object uses this object as its underlay, but it is possible to use it directly .
...
You can learn more of Bird console commands at link:https://bird.network.cz/?get_doc&v=20&f=bird-4.html[Bird Remote Control Doc].

WARNING: Do not apply commands that may alter the Bird configuration while TRex is running with Bird server, only use it for query commands.
