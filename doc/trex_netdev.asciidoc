...
= Test Your Limits With TRex 


== Abstract

Traditionally, network infrastructure devices have been tested using commercial traffic generators for both Stateless and Stateful use cases. The current issue with commercial traffic generators are: 
 ...
 * Extrapolation became a common practice that hides real life bottlenecks and quality issues
 * Delays in development and testing due to testing tools feature dependency
 * Different benchmarks and results methodology, hard to compare results

TRex xref:1[] addresses these implications through an innovative and extendable open source software implementation and by leveraging COTS x86/ARM servers. TRex in a nutshell supports stateful and stateless traffic generation pattern, fueled by DPDK, scale to 200gbps and has automation API. This paper focus on one software design concepts that has been used to achieve scale for Stateful mode
