...
Traditionally, network infrastructure devices have been tested using commercial traffic generators for both Stateless and Stateful use cases. The current issue with commercial traffic are: 

* Cost: Commercial State-full traffic generators are very expensive
...
TRex addresses these problems through an innovative and extendable software implementation and by leveraging standard and open SW and working on COTS x86/ARM server for both Stateless and Stateful use cases. 

In this TCP stack for emulating L7 protocols such as HTTP for router firewall testing.
